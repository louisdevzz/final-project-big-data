...
try:
    from airflow.providers.celery.executors.celery_executor import app as airflow_celery_app
except ImportError:
    from airflow.executors.celery_executor import app as airflow_celery_app

# Re-register all tasks from mycelery.system_worker with Airflow's Celery app
# This allows Airflow workers to handle these custom tasks

@airflow_celery_app.task(name='mycelery.system_worker.run_command', bind=True)
...
@airflow_celery_app.task(name='mycelery.system_worker.docker_compose_up', bind=True)
...
@airflow_celery_app.task(name='mycelery.system_worker.docker_compose_down', bind=True)
...
print("[CeleryTasksPlugin] Registered custom Celery tasks with Airflow's executor")
